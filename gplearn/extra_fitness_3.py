...
def pfl10_longshort_sharpe_simple_with_fee(y, y_pred, w):
    ls_ret, duprate = convert_factor_value_to_returns(y, y_pred, w, _fee_rate, pfl_cnt=10)
    if ls_ret.size and duprate < _duprate_thresh:
        return sharpe_simple(ls_ret, annual_periods=_annual_bar_8h)
    else:
        return _bad_fitness_val
...
_extra_fitness_map = {
    ...
    ## Sharpe simple pfl10 (8h)
    'pfl10_longshort_sharpe_simple_with_fee':  _Fitness(function=pfl10_longshort_sharpe_simple_with_fee, greater_is_better=True), 
